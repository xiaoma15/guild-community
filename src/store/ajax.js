...
instance.interceptors.request.use(function(config) {
    let result = config;
    config.data.requestTime = storage.getTime();
    config.data.onlyone = storage.getUuid();
    result.data = qs.stringify({
        alias: 'ccs.mideaout.' + config.url.replace(new RegExp($conf.baseUrl+'/'),''),
        arg0: JSON.stringify(config.data)
    });
    result.url = result.baseURL;
    return result;
}, function(error) {
...
instance.setDefault = function() {
    ...
    if(user) {
        ...
        // this.defaults.data.employeeType = "40";
...
var pureAjax = axios.create();
pureAjax.interceptors.response.use(function(response) {
    return response.data
}, function(error) {
    if (error.response) {
        // 请求已发送，服务器回复状态码在2xx之外
