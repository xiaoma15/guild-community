...
const _MIDEA_COMMON = 'MideaCommon',
    ...
    _MIDEA_IFLYTEKPLUGIN = 'IFlyTekPlugin'
/**
 * @alias widgetFactory
 * @namespace
...
var functions = {
    ...
    /**
     * 保存相册
     * params: url 需要保存的连接地址
     * @return {*|promise}
     */
    saveToGallery: function () {
        return functions.callApi(_MIDEA_COMMON, 'saveToGallery', [])
    },
    /**
     * 后退
     * @return {*|promise}
    ...
    getPicture: function (params) {
        console.log(storage.get("IS_AD"))
        if (storage.get("IS_AD") == 1 && params.sourceType == 1) {
            var cameraOptions = [30, 0, 1, params.targetWidth, params.targetHeight, 0, 0, false, true, false, null, 0];
            return functions.callApi('Camera', 'takePictureNew', cameraOptions);
        } else {
            // if (params.sourceType == 2) {
            //     return functions.callApi(_MIDEA_COMMON, 'getPicture', [params.maxNum, 1]);
            // } else {
                return new Promise((resolve, reject) => {
                    try {
                        try {
                            ...
                            cameraOptions.destinationType = navigator.camera.DestinationType.FILE_URI; // 返回的图片数据格式  FILE_URI    DATA_URL
                            ...
                            cameraOptions.cameraDirection = 0;
        ...
        // return functions.callApi(_MIDEA_USER, 'takePictureNew', null);

    },
    // 选择照片
    tackPic: function (maxNum) {
        return functions.callApi(_MIDEA_COMMON, 'getPicture', [maxNum, 1]);
    },
    /**
     * 图片uri 压缩
     * @param fields {string} 压缩内容
     * @return {*}
     */
    compressURI: function (params) {
        return functions.callApi(_MIDEA_COMMON, 'compress', params);
    },
        ...
        return new Promise((resolve, reject) => {
            try {
                cordova.exec(function (success) {
                    ...
                    reject(error)
                    // alert("Error: " + error);
                }, "MideaUser", "getContact", params);

            } catch (e) {
                console.log('_error', 'getContact:', e);
                reject(e);
            }
        })
    },
    ...
    getDeviceInfo: function () {
        return functions.callApi(_MIDEA_COMMON, 'getDeviceInfo', []);
    },
    /**
     * 用外部浏览器打开链接
     ...
     */
    speakStart(params) {
        return functions.callApi(_MIDEA_IFLYTEKPLUGIN, 'speakStart', [params]);
    },
    /**
     * @description 停止录音
     * @param params {string}
     ...
     */
    speakStop(params) {
        return functions.callApi(_MIDEA_IFLYTEKPLUGIN, 'speakStop', [params]);
    },
    /**
     * @description 取消录音
     * @param params {string}
     ...
     */
    speakCancel(params) {
        return functions.callApi(_MIDEA_IFLYTEKPLUGIN, 'speakCancel', [params]);
    },
};
